...
.list-left{width: 100px;height: 100px;background: url(/Public/img/katon3.png);float: left;}
	...
	<!-- 订单列表：订单编号、取件编号、下单时间、派送状态，取消订单按钮、确认收货按钮 -->
	        <ul class="ui-list-link">
                <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><li>
                    <div class="list-left" onclick='window.location="/home/user/recordinfo"'>
                    	<div class="subscript" style="background: #12B7F5;">已完成</div>
                    </div>
                    <div class="list-right">
                     <?php echo ($row["ordernum"]); ?>
                     <p>取件号:<?php echo ($row["number"]); ?>
                     <p style="font-size: 13px;">完成时间:<?php echo (date("Y-m-d H:i:s",$row["finishtime"])); ?>
                     <p>
                     	<button class="ui-btn ui-btn-danger order-delete" style="margin-left: 100px;" data-id="<?php echo ($row["id"]); ?>">
		                    删除记录
		                </button>
                    </div>
