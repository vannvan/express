    ...
    <div class="col-lg-3 col-xs-11  col-xs-offset-1 col-lg-offset-9">
       <form>
        <div class="col-xs-8 col-lg-9">
           <input type="text" class="form-control" placeholder="取件号" name="number" value="<?php echo ($_GET['number']); ?>">
        </div>
  ...
  <div class="table-responsive">
		<table class="table table-striped table-hover " id="tableSort">
		  ...
		  <?php if(is_array($rows)): $i = 0; $__LIST__ = $rows;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
		      ...
		      <td>	  
			 	 <button class="btn btn-primary pickit" data-id="<?php echo ($row["id"]); ?>">接单</button>
			  </td>
		    </tr><?php endforeach; endif; else: echo "" ;endif; ?>	
		  </tbody>
		</table>
    </div>
  <?php if(empty($rows)): ?><span>暂无订单！</span>
  <?php else: ?> 
    <button class="btn btn-success" id="ok-btn" type="button">批量接单</button><?php endif; ?> 
   </form>
  </div>
</div>
