<?php 
	...
	function getUserInfo(){
		if(!$_GET['code']){
			...
			$baseinfo=M('Userinfo')->where($condition)->Field('id,nickname,openid')->find();
			if($baseinfo){
				$rst=M('Userinfo')->where($condition)->save($data);
				session('openid',$baseinfo['openid']);
				session('id',$baseinfo['id']);
			}
			else if($data!==NULL){
				$userInfo=D('Userinfo');//实例化数据表
				if(($data['openid'])&&($data['headimgurl'])&&($data['nickname'])){					
					$rst=$userInfo->add($data);
					//获取当前id
	...
	function build_order_no(){
        ...
        //检测是否存在
        $db = M('Order');
        $info = $db->where(array('ordernum'=>$no))->find();
