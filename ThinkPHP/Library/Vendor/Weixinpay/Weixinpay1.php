...
class Weixinpay1 { 
    ...
    public function unifiedOrder($order){
        ...
        $config=array(
            ...
            'nonce_str'=>'X482Q081GQ0UDQS89TGV56U7JQKRX4V7',
        ...
        $result['sign']=$sign;
        $result['nonce_str']='X482Q081GQ0UDQS89TGV56U7JQKRX4V7';
        return $result;
    }
    ...
    **/
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url
            $redirect_uri=U('Weixinpay1/pay','','',true);
            ...
            $total_fee=session('wallet')*100;//价格转换
            $order=array(
                'body'=>'有爱Doer钱包充值',// 商品描述（需要根据自己的业务修改）
                'total_fee'=>$total_fee,// 订单金额  以(分)为单位（需要根据自己的业务修改）
                'out_trade_no'=>$out_trade_no,// 订单号（需要根据自己的业务修改）//坑 这个订单号是测试的固定编号，实际不能重复，微信不允许重复订单号
