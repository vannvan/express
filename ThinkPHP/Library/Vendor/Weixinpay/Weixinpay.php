...
class Weixinpay { 
    ...
    public function unifiedOrder($order){
        ...
        $config=array(
            ...
            'nonce_str'=>'ch5od5jlu32q1wm2mrf2ycnd5khgsy4e',
        ...
        $result['sign']=$sign;
        $result['nonce_str']='ch5od5jlu32q1wm2mrf2ycnd5khgsy4e';
        return $result;
    }
    ...
    **/
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            //$map['ordnum']=$out_trade_no;
            // $orderinfo=M('Order')->where($map)->field('proname,ordnum,paymoney,id')->find();
            //$total_fee=$orderinfo['paymoney']*100;//价格转换
            $OrderInfo=session('cart');
            $total_fee=300;//价格三元 
            $order=array(
                'body'=>'校园快递',// 商品描述（需要根据自己的业务修改）
                'total_fee'=>$total_fee,// 订单金额  以(分)为单位（需要根据自己的业务修改）
                'out_trade_no'=>$OrderInfo['ordernum'],// 订单号（需要根据自己的业务修改）//坑 这个订单号是测试的固定编号，实际不能重复，微信不允许重复订单号
                'product_id'=>$OrderInfo['id'],// 商品id（需要根据自己的业务修改）
                'trade_type'=>'JSAPI',// JSAPI公众号支付
                'openid'=>$openid// 获取到的openid
            );
            //存下订单编号notify要用
            session('ordernum',$OrderInfo['ordernum']);
            // 统一下单 获取prepay_id
